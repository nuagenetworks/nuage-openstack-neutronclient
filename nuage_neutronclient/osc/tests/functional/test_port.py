...
class NuagePortTestsVSDManaged(PortTests):
    ...
    def test_nuage_floating_ip_option(self):
        ...
        self.assertRaisesRegex(Exception, ".*", self.openstack, cmd_create)

        # Verify set / unset
        cmd_set = 'port set --nuage-floatingip 1.1.1.99 {}'.format(port_name)
        # Proper error messages are broken in upstream Queen's
        # We need Change-Id: I617e55d67d93f1e07f5192ba94dcc0997ba9e12f
        # self.assertRaisesRegex(Exception, 'No Nuage FloatingIP available
        # with '
        #                                   'IP 1.1.1.99',
        #                        self.openstack, cmd_set)
        self.assertRaisesRegex(Exception, '.*', self.openstack, cmd_set)
